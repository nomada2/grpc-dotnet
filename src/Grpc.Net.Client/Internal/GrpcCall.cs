        ...
        where TResponse : class
    {
        ...
        private void SetMessageContent(TRequest request, HttpRequestMessage message)
        {
            message.Content = new PushStreamContent(
                (stream) =>
                {
                    var writeMessageTask = stream.WriteMessageAsync<TRequest>(Logger, request, Method.RequestMarshaller.Serializer, Options.CancellationToken);
        ...
        private HttpContentClientStreamWriter<TRequest, TResponse> CreateWriter(HttpRequestMessage message)
        {
            ...
            var writer = new HttpContentClientStreamWriter<TRequest, TResponse>(this, _writeStreamTcs.Task, _writeCompleteTcs);
            return writer;
        }
